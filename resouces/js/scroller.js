...
var RANGEANIMATION = function () {
  ...
  fn.activeAnimation = function (progress) {
    var isDirection = this.checkWheelDirection();
    this.isValue = this.calValue(progress);

    if (this.activeOnStart && !this.activeOnComplate && isDirection == 'down' || this.activeReverseStart && !this.activeReverseComplate && isDirection == 'up') {
      this.onUpdate();
    }

    if (progress >= this.startPoint && !this.complateOnCallback && isDirection == 'down') {
    ...
    if (progress >= this.opts.endPoint && !this.complateOnCallback && isDirection == 'down') {
      if (!this.activeOnComplate) {
        this.onComplate();
    ...
    if (progress <= this.opts.endPoint && !this.activeReverseStart && isDirection == 'up') {
      if (!this.activeReverseStart) {
        this.reverseStart();
        this.activeReverseStart = true;
      }
    }

    if (progress <= this.startPoint && !this.activeReverseComplate && isDirection == 'up') {
      if (!this.activeReverseComplate) {
